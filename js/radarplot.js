...
function RadarChart(id, data, options) {
	...
	var maxValues = {};
	for(var a in cfg.axesFields) {
		maxValues[cfg.axesFields[a]] = 0;
		for (var i in data) {
			d = data[i];
			for (var j in d) {
				if (d[j].axis === cfg.axesFields[a] ) {
					if (d[j].value > maxValues[cfg.axesFields[a]]) {
						maxValues[cfg.axesFields[a]] = d[j].value;
					}
				}
			}
		}
	...
	// var maxValues = [0.2, 1, 0.1];
	rScales = {};
	for (var i in maxValues) {
		rScales[i] = d3.scale.linear()
		.range([0, radius])
	...
	var svg = d3.select(id).insert("svg", ":first-child")
			.attr("width",  cfg.w + cfg.margin.left + cfg.margin.right + 20)
	...
	blobCircleWrapper.selectAll(".radarInvisibleCircle")
		...
		.attr("cx", function(d,i){ return rScales[d.axis](d.value) * Math.cos(angleSlice*i - Math.PI/2); })
		.attr("cy", function(d,i){ return rScales[d.axis](d.value) * Math.sin(angleSlice*i - Math.PI/2); })
		.style("fill", "none")
